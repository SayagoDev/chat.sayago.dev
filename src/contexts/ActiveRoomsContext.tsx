...
interface ActiveRoomsContextType {
  rooms: ActiveRoom[];
  addRoom: (roomId: string, token: string) => void;
  removeRoom: (roomId: string) => void;
  getToken: (roomId: string) => string | null;
  clearRooms: () => void;
}

const ActiveRoomsContext = createContext<ActiveRoomsContextType | undefined>(
  undefined
);

export function ActiveRoomsProvider({
  children,
}: {
  ...
  return (
    <ActiveRoomsContext.Provider
      value={{ rooms, addRoom, removeRoom, getToken, clearRooms }}
    >
      {children}
    </ActiveRoomsContext.Provider>
  );
...
export function useActiveRooms() {
  const context = useContext(ActiveRoomsContext);
  if (context === undefined) {
    throw new Error("useActiveRooms must be used within ActiveRoomsProvider");
  }
  return context;
}
